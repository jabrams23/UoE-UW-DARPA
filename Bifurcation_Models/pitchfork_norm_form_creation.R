...
while (i < 100000) {
	...
	x[i,1] <- 0
	for (j in 2:600) {
		x[i,j] <- x[i,j-1] + h*(r[i,j]*x[i,j-1] + x[i,j-1]^3) + sqrt(h)*rnorm(1, sd=f[i])
	}
	if (length(which(abs(x[i,])>abs(sqrt(abs(r[i,]))))) > 0) {	# check for if tipping has occurred and if so then resmaple the parameters for that specific similation
