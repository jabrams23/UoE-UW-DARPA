...
r0 <- runif(100000,1,5) #initial starting bifurcation parameter
...
while (i < 100000) {
	...
	x[i,1] <- r0[i]
	for (j in 2:600) {
		x[i,j] <- x[i,j-1] + h*(r[i,j]*x[i,j-1] - x[i,j-1]^2) + sqrt(h)*rnorm(1, sd=f[i])
	}
	if (length(which(x[i,] < 0)) > 0) {		# check for if tipping has occurred and if so then resmaple the parameters for that specific similation
